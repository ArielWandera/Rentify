...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(data?.errors?.email?.[0] || data?.message || 'Invalid email or password');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-white dark:bg-dark-base">
        <div className="w-full max-w-md space-y-8">
          ...
          <a
            href="/auth/google"
            className="flex items-center justify-center gap-3 w-full border-2 border-gray-200 rounded-xl py-2.5 text-sm font-semibold text-raisin hover:bg-gray-50 transition"
          >
